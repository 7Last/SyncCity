...
from src.models.raw_data.parking_raw_data import ParkingRawData
...
def test() -> None:
    ...
    rawdata = ParkingRawData(is_occupied=True, latitude=0.0, longitude=0.0, sensor_uuid=UUID("123e4567-e89b-12d3-a456-426614174000"), sensor_name="sensor1", group_name="tullio-levi")

    producer.produce(rawdata)

    client = Client(host="localhost", port=9000, user="admin", password="admin")
    query = "SELECT * FROM sensors.parking"
    ...
    try:
        ...
        assert row[0][6] == rawdata.__is_occupied
