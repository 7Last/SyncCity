...
class ParkingSimulatorStrategy(SimulatorStrategy):
    ...
    def simulate(self) -> ParkingRawData:
        ...
        self._timestamp = self.__generate_next_occupancy_change()
        self.__is_occupied = not self.__is_occupied  # Change the occupancy status
        return data

    def __generate_occupancy(self) -> bool:
        """
    ...
    def __generate_next_occupancy_change(self) -> datetime:
        """
        ...
        hour = self._timestamp.hour
        if 7 <= hour < 9:
            return self._timestamp + self.__random_timedelta(10, 60)
        if 9 <= hour < 18:
            return self._timestamp + self.__random_timedelta(30, 360)
        if 18 <= hour < 22:
            return self._timestamp + self.__random_timedelta(30, 120)

        return self._timestamp + self.__random_timedelta(60, 540)

    def __str__(self) -> str:
        return f'{self.__class__.__name__} {self.__dict__}'

    def __random_timedelta(self, min_minutes: int, max_minutes: int) -> timedelta:
