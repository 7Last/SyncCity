...
class TrafficRawData(RawData):
    def __init__(self, *, vehicles_per_unit: int, avg_speed: float, latitude: float,
                 longitude: float, sensor_id: str,
                 timestamp: datetime = datetime.now()) -> None:
        """:param vehicles_per_unit: number of vehicles passing through the location
         per second
        :param avg_speed: average speed in km/h of the vehicles passing through
        the location
        """
